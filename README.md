...
There are two kinds of time-ordered items on the site:

- **News & Announcements**: these are Jekyll **posts** in the `_posts/` folder.
- **Events**: these are a Jekyll **collection** in the `_events/` folder (rendered on the homepage under “Current and Upcoming Events”).

### News & Announcements (posts)

Each file in `_posts/` is a markdown file with top-matter. Posts are rendered as:

- **A standalone post page** (generated by Jekyll) using the layout `_layouts/news.html` (this is the default for `type: posts` in `_config.yml`).
- **A list entry** on:
  - the homepage (`index.html` → “News & Announcements” list), and
  - the news page (`news.html`).

The content model is intentionally simple: the featured system is **only a list rendering enhancement**. It does not change how posts are written or how post pages are generated.

#### Ordinary internal news post
...
If you want the announcement title to link to an external page, add:

```yaml
redirect: https://example.org/event-page
```

- In lists, the title links to `redirect`.
- The generated post page still exists (normal Jekyll behavior); we simply choose the external link in lists.

#### Featured announcement with poster thumbnail (`featured`, `poster`)

Sometimes it is useful to attach a poster/flyer to a news item (for example, an international event announcement). This is still a normal post; we just render it a little more richly *in lists*.

To show a modest poster thumbnail in lists (homepage/news listing), add:
...
- Put poster images in `assets/posters/`. Use a simple filename (lowercase, hyphens) and include the date. The filename is derived automatically from the post URL/slug:
  - post: `_posts/2026-05-19-ictp-associates-seminars.md`
  - page: `/2026/05/19/ictp-associates-seminars.html`
  - poster: `assets/posters/2026-05-19-ictp-associates-seminars.png`
- This only affects how the item looks inside lists (homepage/news listing). The generated post page remains the same.
...
The homepage list logic lives in `index.html`; the news listing markup lives in `_includes/news_item.html`.
## Navigation
